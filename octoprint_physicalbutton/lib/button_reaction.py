...
def thread_react(pressed_button):
    ...
    bg.plugin._logger.debug(f"Button Toggled: High: {button_value}")
    ...
    if pressed_button.value == button_value:
        ...
        if button.get('buttonType') == 'Toggle':
            if pressed_button.value:
                execActivities(button.get('toggleActivities'))
            else:
                execActivities(button.get('activities'))
        else:
            execActivities(button.get('activities'))

def execActivities(activities):
